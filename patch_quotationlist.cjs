const fs = require('fs');
let file = fs.readFileSync('components/QuotationList.tsx', 'utf-8');
...
file = file.replace(
  `const handleShare = (inv: Quotation, target: 'whatsapp' | 'email' | 'download') => {`,
  `const handleShare = (inv: Quotation, target: 'whatsapp' | 'email' | 'download' | 'drive') => {`
);
...
fs.writeFileSync('components/QuotationList.tsx', file);
