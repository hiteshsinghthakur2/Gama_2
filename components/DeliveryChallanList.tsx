...
const DeliveryChallanList: React.FC<DeliveryChallanListProps> = ({ 
  ...
  const getStatusStyle = (status: DeliveryChallanStatus) => {
    switch (status) {
      case DeliveryChallanStatus.DELIVERED: return 'bg-emerald-100 text-emerald-700';
      case DeliveryChallanStatus.SENT: return 'bg-blue-100 text-blue-700';
      case DeliveryChallanStatus.RETURNED: return 'bg-red-100 text-red-700';
    ...
    if (shareData && (window as any).html2pdf) {
        const generateAndShare = async () => {
            ...
            try {
                ...
                const file = new File([pdfBlob], `Challan-${shareData.doc.number}.pdf`, { type: 'application/pdf' });
                
                const clientName = shareData.client?.name || 'Customer';
                
                // Construct message from template
                const template = userProfile.emailTemplate || "Hi,\n\nPlease find the {type} \"{number}\"\n\nRegards,\n{companyName}";
                const messageText = template
                    .replace(/{type}/g, 'delivery challan')
                    .replace(/{number}/g, shareData.doc.number)
                    .replace(/{amount}/g, '') // No amount in challan usually
                ...
                if (shareData.target === 'download') {
                    ...
                    a.href = url;
                    a.download = `Challan-${shareData.doc.number}.pdf`;
                    ...
                    if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
                        await navigator.share({
                            files: [file],
                            title: `Challan ${shareData.doc.number}`,
                            text: messageText
                        });
                    } else {
                    ...
                    // Email Logic
                     if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
                        ...
                        const subject = `Delivery Challan ${shareData.doc.number} from ${userProfile.companyName}`;
                        ...
                        alert("PDF Downloaded. Please attach the challan to the email.");
                    }
                }
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-1 relative">
                        ...
                        <button 
                          ...
                          className="p-2.5 text-gray-500 hover:bg-gray-100 hover:text-gray-900 rounded-xl transition"
                          title="Edit"
                        >
