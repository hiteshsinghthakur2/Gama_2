...
const checkSharedURLCode = `  const urlParams = new URLSearchParams(window.location.search);
...
if (!content.includes('sharedNoteId = urlParams.get')) {
  content = content.replace(/  return \(\n    <div className="flex h-screen bg-gray-50 overflow-hidden relative print:h-auto print:overflow-visible print:block">/, checkSharedURLCode + "\n    <div className=\"flex h-screen bg-gray-50 overflow-hidden relative print:h-auto print:overflow-visible print:block\">");
}

fs.writeFileSync('App.tsx', content);
console.log('Patched App.tsx for shared URL correctly');
