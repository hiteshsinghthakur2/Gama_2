...
const targetAnchor = `const handleShare = (doc: DeliveryChallan, target: 'whatsapp' | 'email' | 'download') => {`;
content = content.replace(targetAnchor, funcCode + "\n\n  " + targetAnchor);
...
content = content.replace(btnAnchor, btnCode + '\n' + btnAnchor);
