...
export const parseInvoicesBatch = async (files: { base64Data: string, mimeType: string, id: string }[], retries = 3, targetModel = 'gemini-2.0-flash'): Promise<{ success: boolean; data?: any[]; error?: string }> => {
  try {
    const apiKey = getApiKey();
    if (!apiKey || typeof apiKey !== 'string' || apiKey.trim() === "") {
      throw new Error("GEMINI_API_KEY is missing or empty in environment variables. Please add it in your Vercel settings or .env file.");
    ...
    const parts: any[] = [];
    files.forEach((f, idx) => {
      parts.push({ text: `Invoice File ID: ${f.id} (Image ${idx + 1}):` });
      parts.push({
        inlineData: {
          mimeType: f.mimeType,
          data: f.base64Data,
    ...
    parts.push({
      text: "Below are multiple invoices. Extract details for EACH one. Return a JSON array where each object represents an invoice and includes its corresponding 'fileId'. CRITICAL: YOU MUST EXTRACT THE COMPLETE CLIENT DETAILS. Search all over the document, especially 'Billed To', 'Consignee', or 'Buyer'. Capture the Client Name, Email, Phone, and the FULL Address (separated into street, city, state, pincode, country). Also extract invoice number, issue date, due date, client GSTIN, client PAN, whether the client is registered for GST (true/false based on GSTIN presence), place of supply, all line items, additional charges, and terms.",
    });
      ...
      config: {
        ...
        responseSchema: {
          ...
          items: {
            type: Type.OBJECT,
            properties: {
              fileId: { type: Type.STRING, description: 'The File ID specified for this invoice' },
              number: { type: Type.STRING, description: 'Invoice number' },
              date: { type: Type.STRING, description: 'Invoice date (YYYY-MM-DD)' },
              dueDate: { type: Type.STRING, description: 'Due date (YYYY-MM-DD)' },
              clientName: { type: Type.STRING, description: 'Client name' },
    ...
    if ((errorMsg.includes('quota') || errorMsg.includes('exceeded')) && targetModel !== 'gemini-1.5-flash') {
        console.warn(`Model ${targetModel} hit quota limit, falling back to gemini-1.5-flash...`);
        return parseInvoicesBatch(files, retries, 'gemini-1.5-flash');
    }

    if ((errorMsg.includes('429') || errorMsg.includes('503') || errorMsg.includes('UNAVAILABLE') || errorMsg.includes('high demand') || errorMsg.includes('RESOURCE_EXHAUSTED')) && retries > 0) {
      console.warn(`Rate limit or High Demand hit, retrying batch parsing... (${retries} retries left, model: ${targetModel}). Message: ${errorMsg}`);
      ...
      await delay(waitTime);
      return parseInvoicesBatch(files, retries - 1, targetModel);
    }

    console.error(`Gemini AI Service Error (Batch - ${targetModel}):`, errorMsg || error);
    return { success: false, error: errorMsg || 'Unknown processing error' };
  }
};
